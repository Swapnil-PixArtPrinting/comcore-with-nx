...
import { BatchGetSecretValueCommand, FilterNameStringType } from "@aws-sdk/client-secrets-manager";

export class SecretsManagerServiceImpl implements ISecretsManagerServiceInterface {
    ...
    async fetchSecretManagerData(env: string = "all") {
        ...
        console.log(`Fetching secret manager names: ${JSON.stringify(awsSecretNames)}`);
        ...
        do {
            ...
            const response = await this.smClientBuilder.getSecretManagersClient().send(command);
