...
import { isObject, forOwn, unset, cloneDeep } from 'lodash';
...
export class RedactorService {
  private maskValue(
    ...
    switch (strategy) {
      ...
      default:
        ...
        if (len <= 6)
          return strValue[0] + '*'.repeat(len - 2) + strValue[len - 1];
  ...
  redactMultiple(
    obj: any,
    actions: { key: string; action: Action; options?: RedactorOptions }[],
  ): any {
