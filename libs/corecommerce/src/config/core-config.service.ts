...
export class CoreConfigService {
  ...
  async setCoreCommerceClientInstance() {
    CoreConfigService.coreCommerceClients =
      await this.coreCommerceClientBuilder.createClient(
        CoreConfigService.coreCommerceConfig,
      );
  }
   ...
   */
  async getCoreCommerceClientInstance(
    workspace: Workspace,
    clientType: ClientType,
  ): Promise<ICoreCommerceClientWrapper> {
    ...
    if (!clientDetails) {
      throw new Error(
        `No client found for client "${clientType}" in workspace "${workspace}"`,
      );
    }
   ...
   */
  async getCoreCommerceConfigDetails(
    workspace: Workspace,
    clientType: ClientType,
  ): Promise<IBaseCommerceConfig> {
    ...
    if (!clientConfig) {
      throw new Error(
        `No config found for client "${clientType}" in workspace "${workspace}"`,
      );
    }
