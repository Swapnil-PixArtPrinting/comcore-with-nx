...
export class CommercetoolRepository implements CommerceCustomerRepository {
  ...
  async updateAllCustomerActions(
    ...
    try {
      const customerUpdate = new UpdateCustomerDto();
      ...
      return customerDataCollection.body &&
        Object.keys(customerDataCollection.body).length > 0
        ? customerDataCollection.body
        : null;
    } catch (error) {
  ...
  async fetchCustomerByEmail(customerEmail: string): Promise<Customer | null> {
    const customerDataCollection = await this.coreClientService
      ...
      .execute();
    return customerDataCollection.body.count > 0
      ? customerDataCollection.body.results[0]
      : null;
  }
  ...
  async createCustomerFromDraft(customerDraft: CoreRegisterCustomerDTO) {
    ...
    return customerDataCollection.body &&
      Object.keys(customerDataCollection.body).length > 0
      ? customerDataCollection.body.customer
      : null;
  }
}
