...
@Injectable()
export class CommercetoolChannelRepository
  implements CommerceChannelRepository
{
  constructor(
  ...
  async fetchKeyById(channelId: string): Promise<string | null> {
    ...
    const channel =
      channelDataCollection.body &&
      Object.keys(channelDataCollection.body).length > 0
        ? channelDataCollection.body
        : null;
  ...
  async fetchChannelByKey(channelKey: string): Promise<ChannelModel | null> {
    ...
    return channelDataCollection.body &&
      Object.keys(channelDataCollection.body).length > 0
      ? channelDataCollection.body
      : null;
  }
