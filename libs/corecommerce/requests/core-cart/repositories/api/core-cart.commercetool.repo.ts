...
import { Cart, CartUpdateAction, CartUpdate } from '@commercetools/platform-sdk';
...
export class CommercetoolCartRepository implements ICommerceCartRepository {
     ...
     * @param actions 
     */
    async updateCartWithAllActions(cart: Cart, actions: Array<CartUpdateAction>): Promise<Cart | null> {
        try {
            ...
            return response.body && Object.keys(response.body).length > 0 ? response.body : null;
        } catch (error) {
            ...
            if (concurrentError && concurrentError.currentVersion) {
                const updatedCart = {...cart, version: concurrentError.currentVersion}
    ...
    async fetchCartsWhere(email?: string, customerId?: string, page: number = 1, pageSize: number = 10, cartState: string = "Active") : Promise<{ carts: Cart[]; total: number }> {
