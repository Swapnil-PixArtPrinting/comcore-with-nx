...
import { CoreLoggerService } from '../../../common/src';
import {
    ClientType,
...
import { HttpErrorType, HttpMiddlewareOptions, MiddlewareRequest, MiddlewareResponse, Next } from '@commercetools/ts-client/dist/declarations/src/types/types';
...
export class CoreCommerceClientBuilder  {
    ...
    private async createCommercetoolClient(workspace: Workspace, configDetails: ICoreCommerceProviderConfig) {
        ...
        const concurrentModificationMiddlewareOptions: ConcurrentModificationMiddlewareOptions = {
            concurrentModificationHandlerFn: (version, request) => {
                ...
                this.loggingService.error(["Error"], `Concurrent modification error, retry with version ${version}`, body, body);
            ...
            for (const clientType in workspaceConfig) {
                ...
                switch (clientType) {
                    case dataClient.COMMERCETOOL:
                        if(configDetails)
                            await this.createCommercetoolClient(workspaceKey as Workspace, configDetails);
                        break;
                    default:
